...
output_filename = 'slam_bounce_zoom_effect.mp4' # Updated filename
...
zoom_duration_sec = 0.25  # << NEW: Duration for the final subtle zoom

num_bounces = 2.0         # How many full up-and-down bounces (can be float for partial)
bounce_height_initial = 0.2 # Initial bounce height as a factor of image height (e.g., 0.2 = 20%)
squash_factor_max = 0.85  # How much it squashes vertically at impact (e.g., 0.85 = 85% height)
damping_factor = 4.0      # How quickly the bounce dampens out (higher = faster stop)
zoom_scale_factor_end = 1.05 # << NEW: Final scale factor (1.05 = 5% zoom in). Keep close to 1.0 for minimal zoom.

# --- End Configuration ---

# --- Load Image ---
print(f"Loading image from: {img_path}")
img_orig_raw = cv2.imread(img_path, cv2.IMREAD_UNCHANGED) # Load raw first
...
def get_image_components(img_to_process):
    ...
    if len(img_to_process.shape) == 3 and img_to_process.shape[2] == 4: # Check for 3 dims and 4 channels
        bgr = img_to_process[:, :, 0:3].copy() # Make copies
        ...
        bgr = img_to_process.copy() # Make copy
        ...
        return bgr, alpha_mask
         ...
         bgr = cv2.cvtColor(img_to_process, cv2.COLOR_GRAY2BGR) # Creates a new BGR image
         alpha_mask = np.ones((h, w, 3), dtype=np.float32)
         return bgr, alpha_mask
    else:
...
num_zoom_frames = max(1, int(fps * zoom_duration_sec)) # << NEW
total_frames = num_drop_frames + num_bounce_frames + num_zoom_frames # << UPDATED

start_y = -img_h_orig # Start fully off-screen vertically
end_y = 0            # Final resting position (top of image at top of canvas)
center_x_orig = 0    # Image horizontally fills the canvas (assuming image width == canvas width)
# If image width < canvas width, use: center_x_orig = (canvas_w - img_w_orig) // 2

# Calculate max bounce in pixels
...
# --- Animation Loop ---
print(f"Generating {total_frames} frames ({num_drop_frames} drop + {num_bounce_frames} bounce + {num_zoom_frames} zoom)...")
background = np.zeros((canvas_h, canvas_w, 3), dtype=np.uint8) # Black background
start_time = time.time()

# Pre-calculate original image center X for positioning deformed image
...
for frame_num in range(total_frames):
    ...
    pos_x = center_x_orig # Default horizontal position
    pos_y = end_y         # Default vertical position after drop/bounce

    # --- Phase Logic ---
    if frame_num < num_drop_frames:
        # Phase 1: Dropping (No deformation)
        phase = "Dropping"
        ...
        # No resize needed here, dimensions are original

    elif frame_num < num_drop_frames + num_bounce_frames:
        ...
        # Calculate bounce height using a dampened sine wave
        ...
        # Calculate current vertical position (bottom of image)
        current_y_bottom = end_y + img_h_orig # Target bottom position
        pos_y_bottom = current_y_bottom - current_bounce_offset # Bouncing bottom
        ...
        # Calculate top-left corner position for drawing
        pos_y = pos_y_bottom - current_h
        pos_x = center_x_orig + (img_w_orig - current_w) // 2 # Keep original center aligned

    else:
        # Phase 3: Zooming In (after bounce settles)
        phase = "Zooming"
        zoom_frame_num = frame_num - (num_drop_frames + num_bounce_frames)
        zoom_progress = zoom_frame_num / (num_zoom_frames - 1) if num_zoom_frames > 1 else 1.0

        # Linear interpolation for scale factor from 1.0 to zoom_scale_factor_end
        current_scale_factor = 1.0 + (zoom_scale_factor_end - 1.0) * zoom_progress
        ...
        # Start from the final resting position (end_y, center_x_orig) and adjust
    ...
    # --- Drawing the Image ---
    # (This section remains largely the same, using the calculated
    #  pos_x, pos_y, current_w, current_h, img_to_draw_bgr, img_to_draw_alpha)
       ...
       y_start_img < y_end_img and x_start_img < x_end_img:
        ...
        if h_roi == h_img_slice and w_roi == w_img_slice:
            try:
                ...
                # Fallback
                try: # Attempt simple copy as fallback
