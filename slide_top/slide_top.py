...
def slide_top_effect(image_path, duration=5, scale_factor=1.2, fps=25):
    """
    Creates a MoviePy VideoClip that applies a slide_top animation
    on the given image.
    
    The image is first scaled up by 'scale_factor' to provide extra vertical pixels.
    Then a crop window of the original size (w x h) is moved vertically from the top edge
    (y_offset = 0) to the bottom edge (y_offset = new_h - h) over the duration.
    ...
    :return:             A MoviePy VideoClip with the slide_top animation.
    """
    ...
    def make_frame(t):
        ...
        progress = t / duration
        # For slide_top: at t=0, y_offset = 0; at t=duration, y_offset = new_h - h.
...
if __name__ == "__main__":
    ...
    # Process each image: create a slide_top animation and save as an MP4
    for img_path in image_files:
        try:
            # Generate the slide_top clip for each image
            clip = slide_top_effect(
                image_path=img_path,
                duration=5,       # 5-second animation
                scale_factor=1.2, # Scale up to 120% to allow vertical slide
                fps=25
            )
            # Construct output filename (e.g., original_name_slide_top.mp4)
            base_name, _ = os.path.splitext(os.path.basename(img_path))
            output_video_path = os.path.join(downloads_folder, base_name + "_slide_top.mp4")
